...
export default class Captcha {
  ...
  constructor(options: IConfig = {} as IConfig) {
    ...
    const { imageW, imageH, length, fontSize } = tempConfig;
  ...
  drawNoiseText() {
    const ctx = this.canvasContext;
    const { fontFamily, imageW, imageH, codeSet } = this.config;
    for (let i = 0; i < 10; i++) {
      // 杂点颜色
   ...
   */
  randomChars(): string {
    const { useZh, codeSet, zhSet, length } = this.config;
