...
export default abstract class AuthService extends Service {
  ...
  public async handleActiveAccountEmail (email: string, result: any) {
    // 生成 token
    let token = await this.createRestPasswordToken(email)
  ...
  public async handleSendPasswordResetEmail (email: string) {
    // 获取 emial 的请求
    let result = await this.repository.getUserByEmail(email)
  ...
  public async verifyRestPasswordToken (email: string, token: string) {
    let result = await this.passwordResetRepository
    ...
    let isOverTime = (moment().unix() - moment(result.created_at).unix()) > (1000 * 60 * 60 * 24)
