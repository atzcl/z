...
export default class User extends BaseHandler {
   ...
   */
  public async get (openId: string, lang: string = 'zh_CN') {
    let params = {
      openid: openId,
      lang: lang
    }

    let result = await this.ctx.handlers.wechat.request.get('user/info', { data: params })
   ...
   * @param {string} lang  返回国家地区语言版本，zh_CN 简体，zh_TW 繁体，en 英语
   */
  public async select (openIds: string[], lang: string = 'zh_CN') {
