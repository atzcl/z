...
export default class Jwt extends BaseHandler {
  ...
  public async verify (token: string) {
    // 获取解密成功的数据
    let result = await this.app.jwt.verify(token, this.app.config.jwt.secret, {
  ...
  public async getSub (token: string) {
    let jwtData: any = await this.verify(token)
  ...
  public async blackToken (token: string) {
    ...
    // 解密 token
    let jwtData = await this.decode(token)

    // 获取 token 失效时间
    let refreshTtl = jwtData.payload.iat + (this.app.config.jwt_extra.refresh_ttl * 60 * 60)

    // 获取储存到缓存的失效时间
    let refreshTtlTime = refreshTtl - Math.floor(new Date().getTime() / 1000)
