...
export default class WechatUserService extends Service {
  ...
  public async login (): Promise<string> {
    ...
    // 到微信服务器查询该用户的详情
    const userInfo = await this.getWeChatUserInfo(this.getRequestBody.FromUserName);

    // 获取当前微信用户的在库请求
    const result = await ctx.wechatRepository.wechatUser.scanQrCodeLogin(
      {...userInfo,  app_id: this.getRequestBody.ToUserName}, // 添加 app_id
    );
  ...
  public async getEventKey (evn: string) {
    ...
    const realEventKey = evn.split('qrscene_');
